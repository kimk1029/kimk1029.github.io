...
export default function Portfolio() {
   ...
   * * 구간 정의:
   * 0.00 ~ 0.20 : [Section 1] Hero - 우측 상단 대기 -> 중앙 하강
   * 0.20 ~ 0.40 : [Section 2] Career - "현재" 프로젝트(좌측 상단) 가리킴
   * 0.40 ~ 0.65 : [Section 3] About (Text) - 텍스트 사이를 지그재그로 비행
   * 0.65 ~ 0.75 : [Section 3] About (Stats) - 숫자 8과 13 주변을 8자 비행
   * 0.75 ~ 0.90 : [Section 4] Projects - 중앙 정렬 후 수직 하강
   * 0.90 ~ 1.00 : [End] - 루프(Loop) 후 착륙
   */

  // X축 움직임 (vw 단위)
  const planeX = useTransform(smoothScroll,
    [0, 0.2, 0.25, 0.45, 0.50, 0.55, 0.60, 0.68, 0.72, 0.80, 0.95, 1.0],
    ["80vw", "50vw", "15vw", "15vw", "80vw", "10vw", "80vw", "30vw", "70vw", "50vw", "50vw", "50vw"]
    // Hero -> Center -> Career(Left) -> Stay -> Text(Right) -> Text(Left) -> Text(Right) -> Stat(8) -> Stat(13) -> Center -> Down -> Loop
  );

  // Y축 움직임 (vh 단위 - 고정된 뷰포트 기준 위치)
  const planeY = useTransform(smoothScroll,
    [0, 0.2, 0.25, 0.40, 0.45, 0.50, 0.55, 0.60, 0.70, 0.80, 0.95, 0.98, 1.0],
    ["15vh", "80vh", "35vh", "40vh", "20vh", "35vh", "50vh", "65vh", "75vh", "20vh", "90vh", "85vh", "95vh"]
    // Hero -> Transition -> Career(Top Item) -> ... -> Text Lines -> Stats -> Projects Top -> Bottom -> Loop Up -> Land
  );

  // 회전 (Rotate) - 진행 방향에 맞춰 기수 돌리기
  const planeRotate = useTransform(smoothScroll,
    [0, 0.15, 0.2, 0.25, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.80, 0.99, 1.0],
    [0, 45, 135, 10, 45, 160, 20, 160, -45, 45, 180, 180, 0]
  );
    ...
    <div ref={containerRef} className="bg-black text-slate-300 selection:bg-blue-500/30 overflow-x-hidden perspective-1000 relative">
      ...
      <div className="relative z-10">
        ...
        {/* =========================================================================
            ...
            Height: 100vh (꽉 찬 화면) - 비행기가 "현재"를 가리킴
           ========================================================================= */}
          ...
          <div className="max-w-6xl mx-auto w-full h-full flex flex-col justify-center">
            ...
            <div className="relative border-l-2 border-slate-800 ml-4 md:ml-0 md:border-l-0">
              ...
              <div className="relative md:grid md:grid-cols-2 gap-12 mb-12 items-start">
                ...
                {/* Right: Empty for balance in this layout, or add more details */}
        ...
        {/* =========================================================================
            ...
            Height: 100vh (꽉 찬 화면) - 비행기가 텍스트를 읽고 숫자를 돔
           ========================================================================= */}
        ...
        {/* =========================================================================
            ...
            Normal Scroll - 비행기가 중앙에서 아래로 하강
           ========================================================================= */}
